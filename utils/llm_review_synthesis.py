...
NOVELTY_SCORE_FUNC = lambda best_sim: max(
    0, min(10, int(round((1.0 - best_sim) * 10)))
)  # best_sim in [0,1]
NOVELTY_WARN_THRESHOLD = 0.4  # if best_sim >= 0.6 we should warn (i.e. not novel)
PLAGIARISM_SCORE_THRESH = (
    0.7  # paraphrase similarity >= this is considered high paraphrase overlap
)
EXACT_MATCH_THRESHOLD = (
    0.995  # treat >= this as an exact/near-exact match -> auto-reject
)
...
def top_k_overlaps(plagiarism_json, k=TOP_EVIDENCE):
    ...
    if not plagiarism_json:
        return []
    overlaps = []
    for o in plagiarism_json.get("paraphrase_overlap", []) + plagiarism_json.get(
        "exact_overlap", []
    ):
        # unify fields
        ...
        overlaps.append(
            {
                ...
                "pdf_path": pdf_path,
                "snippet": snippet,
                "type": o.get("type", "paraphrase_overlap"),
            }
        )
...
def top_k_duplicate_claims(claims_json, k=TOP_EVIDENCE):
    ...
    dups = [
        m
        for m in claims_json["mappings"]
        if m.get("similarity", 0.0) >= EXACT_MATCH_THRESHOLD
    ]
    # if none exact, show top by similarity
    if not dups:
        mappings = sorted(
            claims_json["mappings"],
...
def compute_plagiarism_summary(plag_json):
    ...
    has_exact = any(
        float(x.get("score", x.get("similarity", 0.0))) >= EXACT_MATCH_THRESHOLD
        for x in parap + exact
    )
    return {
        "paraphrase_count": len(parap),
...
def build_final_recommendation(
    scores, strengths, weaknesses, suggestions, plagiarism_summary, claim_dups
):
    """
    ...
    if plagiarism_summary.get("has_exact") or (
        claim_dups
        and any(m.get("similarity", 0.0) >= EXACT_MATCH_THRESHOLD for m in claim_dups)
    ):
        return dedent(
            """\
...
def synthesize_report(paper_dir: Path, output_file: Path, dry_run=False):
    ...
    cit_score = (
        citation.get("analysis", {}).get("citation_quality_score") if citation else None
    )
    if cit_score is not None:
        if cit_score >= 7:
            ...
            weaknesses.append(
                "Citation quality is low or many missing DOIs/outdated references."
            )
            suggestions.append(
                "Update references and include more recent and diverse citations."
            )
    else:
    ...
    if nov_stats["num_results"] == 0:
        ...
        if best_sim >= EXACT_MATCH_THRESHOLD:
            weaknesses.append(
                f"Top similarity = {best_sim:.3f} (near-exact match with an existing paper)."
            )
            suggestions.append(
                "Rewrite sections identical to prior work and clearly cite them."
            )
        elif best_sim >= PLAGIARISM_SCORE_THRESH:
            weaknesses.append(
                f"High similarity found (top similarity = {best_sim:.3f})."
            )
            suggestions.append(
                "Clarify novelty and explicitly contrast contributions vs. the closest prior art."
            )
        else:
            strengths.append(
    ...
    if not plagiarism:
        ...
        if plag_sum["paraphrase_count"] + plag_sum["exact_count"] == 0:
            ...
            weaknesses.append(
                f"{plag_sum['paraphrase_count'] + plag_sum['exact_count']} plagiarism/paraphrase overlaps detected."
            )
            # add evidence bullet
            top_ov = plag_sum["top_overlaps"]
            if top_ov:
                suggestions.append(
                    "Key overlaps found — see evidence list in report; consider rewriting those parts."
                )
    # Factual reasoning
    factual_issues = 0
    if factual:
        ...
        if factual_issues > 0:
            ...
            suggestions.append(
                "Correct factual inconsistencies and double-check units/values."
            )
        else:
    ...
    if claims:
        for m in claims.get("mappings", []):
            ...
            label = (
                "FALSE" if is_duplicate else "TRUE"
            )  # FALSE => not novel / potentially unsupported as original
            claim_label_lines.append(
                f"{label} → {m.get('claim')[:180]} (sim={m.get('similarity')})"
            )
    else:
        claim_label_lines = [
    ...
    final_reco = build_final_recommendation(
        scores, strengths, weaknesses, suggestions, plag_sum, claim_dups
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Synthesize review report from pipeline artifacts"
    )
    parser.add_argument(
        "--paper_dir",
        type=str,
        required=True,
        help="Directory with intermediate results (citation_report.json, novelty.json, plagiarism.json, factual.json, claim_mapping.json)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output review file (defaults to paper_dir/review.txt)",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Print the report instead of saving"
    )
