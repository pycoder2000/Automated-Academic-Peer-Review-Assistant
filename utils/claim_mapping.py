...
def map_claims(
    new_claims, existing_claims, model, claim_threshold=DEFAULT_CLAIM_SIM_THRESHOLD
):
    if not new_claims:
        return []
...
def main():
    ...
    parser.add_argument(
        "--similar_json", required=True, help="Path to novelty.json (similar papers)"
    )
    parser.add_argument(
        "--claim_threshold",
        type=float,
        default=DEFAULT_CLAIM_SIM_THRESHOLD,
        help="Similarity threshold",
    )
    parser.add_argument(
        "--model", type=str, default=MODEL_NAME, help="SentenceTransformer model"
    )
    parser.add_argument(
        "--out_dir", required=False, help="Output directory for results"
    )  # NEW
    ...
    print(
        f"[STEP5] Collected {len(existing_claims)} claims from {len(similar_list)} similar papers."
    )
    ...
    mappings = map_claims(
        new_claims, existing_claims, model, claim_threshold=args.claim_threshold
    )
    ...
    for m in mappings:
        ...
        if m.get("matched_claim"):
            print(
                "Matched paper:",
                m.get("matched_paper_title", "N/A"),
                "| Matched claim:",
                m["matched_claim"],
            )
        else:
            print("No matching existing claims found (claim is novel)")
