...
import ReviewPaperPage from './components/ReviewPaperPage';
...
function App() {
    ...
    const handleViewReviewPaper = (submission: ReviewSubmission) => {
        setSelectedPaper(submission);
        setCurrentPage('view-paper');
        window.scrollTo({ top: 0, behavior: 'smooth' });
    };

    const handleBackToReviewPapers = () => {
        setCurrentPage('review-paper');
        window.scrollTo({ top: 0, behavior: 'smooth' });
    };
    ...
    const handleLogout = () => {
        // Logout first
        auth.logout();
        // Immediately navigate to landing page and scroll to top
        setCurrentPage('landing');
        window.scrollTo({ top: 0, behavior: 'instant' });
        // Dispatch storage event to notify Navbar and other components
        window.dispatchEvent(new Event('storage'));
        // Also dispatch a custom logout event
        ...
        <div className="App">
            ...
            {currentPage === 'login' ? (
                ...
                <ViewPaperPage
                    paper={selectedPaper}
                    onBack={
                        // Determine which back handler to use based on paper type
                        (selectedPaper as any).submission_id
                            ? handleBackToReviewPapers
                            : handleBackToPapers
                    }
                    onReviewPaper={() => {
                        // Placeholder - will be implemented later
                        if ((selectedPaper as any).submission_id) {
                            console.log('Review submission:', (selectedPaper as any).submission_id);
                        } else {
                            console.log('Review paper:', (selectedPaper as Publication).publication_id);
                        }
                    }}
                />
            ) : currentPage === 'landing' ? (
